...
import { HouseRulesService, HouseRulesError } from "./house-rules.js";
...
describe("HouseRulesService", () => {
  ...
  it("refuses a write over the cap instead of truncating it", async () => {
    ...
    expect(existsSync(service.path("global"))).toBe(false);
    await service.write("global", "x".repeat(HOUSE_RULES_MAX_CHARS));
    expect((await readFile(service.path("global"), "utf8")).trim()).toHaveLength(HOUSE_RULES_MAX_CHARS);
  });

  it("clamps a hand-edited file past the cap at injection", async () => {
    const path = service.path("project", "p1");
  ...
  it("deletes the file when saved empty", async () => {
    ...
    expect(existsSync(service.path("project", "p1"))).toBe(false);
  });
});
