...
export function registerChatRoutes(app: FastifyInstance): void {
  ...
  // Through `deleteChat` — the same path the retention sweep uses — because
  // `store.deleteChat` alone strands the session, its processes and its refs.
  app.delete<{ Params: { id: string } }>(
    "/api/chats/:id",
    async (req, reply) => {
      await deleteChat({ ...app.services, store, bus }, req.params.id);
